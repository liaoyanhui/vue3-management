 ...
 * @LastEditTime: 2023-11-17 09:38:27
-->
<template>
  <BasicModal
    v-bind="$attrs"
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      const [registerTable, { getForm, reload }] = useTable({
        api: (params) => {
          return memberStore.getOrderStockPagedList({
            ...params,
            userId: userId.value,
          });
        },
      ...
      function handleOpen() {
        memberStore.exportOrderStockData({ userId: userId.value }).then((res) => {
          excelData.value = res || [];
          openModal();
        });
      }
