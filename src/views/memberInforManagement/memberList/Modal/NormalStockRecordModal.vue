 ...
 * @LastEditTime: 2023-07-26 15:15:17
-->
<template>
  <BasicModal
    v-bind="$attrs"
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      const [registerTable, { getForm, reload }] = useTable({
        api: (params) => {
          return memberStore.getNormalStockPagedList({
            ...params,
            userId: userId.value,
          });
          // const items = result.items;
          // excelData.value = items;
          // return result;
        },
      ...
      async function handleOpen() {
        memberStore.exportNormalStockData({ userId: userId.value }).then((res) => {
          excelData.value = res || [];
          openModal();
        });
